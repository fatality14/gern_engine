...
layout (location = 3) in vec3 vertex_joints;
...
void main()
{
    ...
    for(int i = 0; i < 3; i++){
        vec4 jointPosShift = jointTransforms[int(vertex_joints[i])] * vec4(vertex_position, 1.0);
        newVertexPos += jointPosShift * vertex_weights[i];

        vec4 jointNormShift = jointTransforms[int(vertex_joints[i])] * vec4(vertex_normal, 0.0);
