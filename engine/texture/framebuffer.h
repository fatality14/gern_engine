...
public:
    ...
    ~Framebuffer() {
        delete textureColorBuffers; // a bug if someone takes TextureList
                                    // control and clear it
    ...
    void appendTextureColorBuffers(size_t amount) {
        ...
        for (size_t i = tcbSize; i < tcbSize + amount; i++) {
            ...
            textureColorBuffers->appendTextureToLayout(
                0, i, i, "textureColorBuffer" + to_string(i));

            genColorAttachmentFramebuffer(i);
        }
        attachRenderBuffer();
    }
    void bind() { GLDB(glBindFramebuffer(GL_FRAMEBUFFER, FBO)); }
...
private:
    void genColorAttachmentFramebuffer(GLuint attachmentNum) {
        ...
        GLDB(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB,
        ...
        GLDB(glFramebufferTexture2D(
            GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + attachmentNum, GL_TEXTURE_2D,
            textureColorBuffers->at(attachmentNum)->textureId, 0));
    }
    void attachRenderBuffer() {
        GLDB(glBindRenderbuffer(GL_RENDERBUFFER, RBO));
        GLDB(glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, width,
                                   height));

        GLDB(glFramebufferRenderbuffer(
